...
Decision: stage 7B repositions the product as Alune Knowledge and adds the first RAG knowledge-base module: knowledge bases, document ingestion, pgvector chunk indexing, and single-turn knowledge Q&A with citations. The foundation is ready to harden RAG quality in narrow follow-up stages.

## Scope

This document is a feature-readiness check for Alune Knowledge. It does not introduce OCR, approval flows, reports, payroll, employee workflows, executable scripts, task scheduling, dynamic plugin loading, or other company-specific enterprise workflows.
...
| RAG Knowledge V1                | Ready                          | Stage 7B adds knowledge-base routes, pgvector tables, document parsing/chunking/indexing, OpenAI-compatible AI settings, and `/api/v1/rag/ask` citation responses.                 | Keep V1 limited to PDF/DOCX/TXT/Markdown and single-turn Q&A; add OCR, background queues, and retrieval tuning only in scoped stages. |
...
- `apps/api/app/modules/users/`, `roles/`, `departments/`, `apps/`, `dictionaries/`, `files/`, and `knowledge/` routers are still understandable, but new module logic should avoid growing existing platform-management routers.
...
1. Stage 7C: harden RAG quality with scoped retrieval tuning, index retry UX, and better document status feedback.
2. Keep OCR, approval flows, payroll, reports, script execution, schedulers, and broad workflow engines out until a narrow module needs them.
