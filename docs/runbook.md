...
The generation command writes `packages/api-client/openapi/openapi.json` through `apps/api/app/scripts/export_openapi.py`, then Orval writes `packages/api-client/src/generated/api.ts`. The export script normalizes multipart binary fields to `format: binary` so generated upload bodies use `Blob`. Dashboard health, auth entry points, and internal-system read queries already use generated hooks; mutation-heavy actions still use the compatibility client in `packages/api-client/src/index.ts` and should be migrated incrementally to `@alune/api-client/generated`.
