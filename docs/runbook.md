...
The generation command writes `packages/api-client/openapi/openapi.json` through `apps/api/app/scripts/export_openapi.py`, then Orval writes `packages/api-client/src/generated/api.ts`. The export script normalizes multipart binary fields to `format: binary` so generated upload bodies use `Blob`. Home health, auth entry points, platform read queries, and user/role/space/dictionary JSON write actions already use generated hooks. The root compatibility client in `packages/api-client/src/index.ts` is now limited to runtime configuration, audit CSV export, file upload, file download, and the types needed by those binary/CSV boundaries.
