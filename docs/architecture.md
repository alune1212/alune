...
`alune-platform` is a minimal monorepo foundation for a company internal admin platform. The current implementation stops at the stage 6A MVP: infrastructure, health checks, a dashboard shell, Alembic-managed tables, a narrow login flow, the first RBAC baseline, and basic internal system pages for users, roles, and departments.
...
- `app/modules/users/`, `app/modules/roles/`, and `app/modules/departments/` implement the stage 6A internal system API.
- `app/modules/audit/`, `app/modules/dictionaries/`, and `app/modules/files/` currently provide database models for later UI/API expansion.
...
- Users, roles, and departments pages in `src/features/users/`, `src/features/roles/`, and `src/features/departments/`.
...
- Full user CRUD, role permission editor, audit log UI, dictionary UI, file upload/download flow, approvals, reports, and company-specific business modules.
